...
# Remove any existing new_style if I already inserted it
# Then insert standard valid `<style>` tag text content inside `{""}` using string concat, or better: just CSS, and JSX allows writing literal text, EXCEPT in curly braces `{}` 

style_regex = re.compile(r'<style>.*?</style>', re.DOTALL)
text = style_regex.sub("""<style> {".custom-scrollbar::-webkit-scrollbar { width: 8px; height: 8px; }" + ".custom-scrollbar::-webkit-scrollbar-track { background: transparent; }" + ".custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.05); border-radius: 4px; }" + ".custom-scrollbar::-webkit-scrollbar-thumb:hover { background: rgba(255,255,255,0.1); }" + "@keyframes slide { 0% { transform: translateX(-100%); } 100% { transform: translateX(300%); } }" + "@keyframes slideLeft { from { transform: translateX(100%); } to { transform: translateX(0); } }" + "@keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }"} </style>""", text)
